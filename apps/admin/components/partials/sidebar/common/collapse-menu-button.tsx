...
import {useMobileMenuConfig, useMenuHoverConfig} from "@/hooks/use-config";
			...
			<Collapsible open={isCollapsed} onOpenChange={setIsCollapsed}>
				<CollapsibleTrigger asChild>
					...
					>
						...
						<p
							className={cn("max-w-50 text-wrap! text-[11px] truncate ")}
						>
							{label}
						</p>
					</Button>
				...
				<CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
					{submenus.map(({href, label, active}, index) => (
						<Button
							...
							size="sm"
							className={cn(
								"w-full justify-center text-wrap text-center p-0  h-auto  hover:bg-transparent hover:text-default capitalize text-xs font-normal mb-2 first:mt-4 last:mb-0",
								{
