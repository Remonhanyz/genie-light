	...
	return (
		...
		>
			<CollapsibleTrigger className="mb-1" asChild>
				<div className="flex items-center group [&[data-state=open]>button>div>svg]:rotate-180">
					<Button
					...
					>
						<div className="w-full items-center flex justify-between">
							...
							<p className={cn("max-w-[150px]")}>{label}</p>
						</div>
				...
				{submenus.map(({href, label, active}, index) => (
					...
					>
						<Link href={href}>
							<span
								className={cn(
									"h-1.5 w-1.5 me-3 rounded-full bg-default transition-all duration-150 ring-0   bg-default-300 dark:bg-default   dark:ring-menu-arrow-active ring-secondary-foreground",
									{
